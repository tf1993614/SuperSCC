...
# run get_meta_program
gene_module = get_meta_program(ensemble_id_files, parallel_num = 32)
...
# modules = modules[c(1, 7, 2, 5, 3, 4, 8, 6, 9, 10, 11)]
# robust_candidates = gene_module[[2]][map_lgl(gene_module[[2]], ~ length(.x) > 2)]
# modules = modules[map_int(modules, ~ length(unlist(robust_candidates[.x]))) %>% order() %>% rev()]
...
# c(54, 55, 63, 62)
#
# selection = which(colnames(intersection) %in% c(30, 27, 29, 31, 28, 50,
#                                                 51, 53, 54, 55, 57, 61,
#                                                 59, 62, 63, 25, 32, 39,
#                                                 44, 48, 65))
# intersection = intersection[selection, selection]

# only focus on the F modules
# which backtracks to M module 3
# selection = as.character(c(1, 10, 11, 13, 14, 15, 16,
#               17, 19, 2, 21, 22, 23, 24,
#               3, 4, 5, 6, 8, 9, 27, 29, 30,
#               31, 25, 32, 50, 51, 54, 55,
#               57, 59, 61, 62, 63, 39, 44,
#               48))
#
# selection_order = which(colnames(intersection) %in% selection)
# intersection = intersection[selection_order, selection_order]
